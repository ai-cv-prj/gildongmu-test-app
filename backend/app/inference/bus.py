"""버스 실제 모델 파이프라인. 버스 담당자가 수정하는 파일이다.

가중치 파일은 backend/models/bus/ 폴더에 넣는다. 확장자가 .pt .pth .onnx .engine 인 파일은
...
점검: .venv/bin/python scripts/check_model.py --mode bus --image 사진.jpg
"""
...
class BusPipeline:
    ...
    def load(self) -> None:
        ...
        raise NotImplementedError("backend/app/inference/bus.py 의 load() 에 모델 로딩 코드를 넣어주세요")
    ...
    def infer(self, frame_bgr: np.ndarray, context: InferenceContext) -> dict[str, Any]:
        ...
        #           "extra": {},
        #       })
        raise NotImplementedError("backend/app/inference/bus.py 의 infer() 에 추론 코드를 넣어주세요")
        return {"detections": detections, "event": {"type": "bus_detection", "bus_number": None, "is_target": False}}
