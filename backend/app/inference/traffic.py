"""신호등 실제 모델 파이프라인. 신호등 담당자가 수정하는 파일이다.

가중치 파일은 backend/models/traffic/ 폴더에 넣는다. 확장자가 .pt .pth .onnx .engine 인 파일은
...
점검: .venv/bin/python scripts/check_model.py --mode traffic --image 사진.jpg
"""
...
class TrafficPipeline:
    ...
    def load(self) -> None:
        ...
        raise NotImplementedError("backend/app/inference/traffic.py 의 load() 에 모델 로딩 코드를 넣어주세요")
    ...
    def infer(self, frame_bgr: np.ndarray, context: InferenceContext) -> dict[str, Any]:
        ...
        #           "extra": {},
        #       })
        raise NotImplementedError("backend/app/inference/traffic.py 의 infer() 에 추론 코드를 넣어주세요")
        # signal_state 는 red / green / unknown 중 하나
        return {"detections": detections, "event": {"type": "traffic_signal", "signal_state": "unknown"}}
