"""
file_path: tests/backend/api/test_api.py
...
# 보행 Mock에는 YOLO 설정이 없으므로 공통 기본값을 적용하는지 확인한다.
def test_walking_mock_uses_common_confidence_default(client: TestClient):
    """
    신뢰도를 생략한 보행 Mock 세션에는 공통 기본값 0.4가 기록된다.
    """
    session_id = start(client, "walking")
    assert client.app.state.service._active.confidence == 0.4
    session = client.get(f"/api/sessions/{session_id}").json()
    assert session["settings"]["confidence"] == 0.4
