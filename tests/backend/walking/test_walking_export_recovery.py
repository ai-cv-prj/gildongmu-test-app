"""
file_path: tests/backend/walking/test_walking_export_recovery.py
...
import cv2
import numpy as np
from backend.app.services.walking.walking_export import WalkingExporter,read_status,export_video,validate_video
from backend.app.services.walking.walking_storage import save_input,save_risk,write_json
...
def test_encoder_failure_retry_and_published_output_recovery(walking):
    ...
    assert upload(c,sid,1).status_code==200
    c.post(f'/api/sessions/{sid}/stop')
    with patch('backend.app.services.walking.walking_export.export_video',side_effect=RuntimeError('encoder unavailable')):
...
def test_raw_storage_failure_never_runs_inference(walking):
    c,svc,p,sid,d=walking
    with patch('backend.app.services.walking.walking_frames.save_input',side_effect=OSError('disk full')):
        r=upload(c,sid,1)
